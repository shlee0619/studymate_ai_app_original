...
export const ReviewScreen: React.FC = () => {
  ...
  const handleReviewResponse = async (correct: boolean) => {
    ...
    // SM-2 알고리즘 적용
    let newEF = currentItem.ef;
    let newReps = currentItem.reps;
    let newInterval = currentItem.intervalDays;
    ...
    if (correct) {
      ...
      if (newReps === 1) {
        ...
        newInterval = Math.round(currentItem.intervalDays * currentItem.ef);
      }
      newEF = currentItem.ef + 0.1 - (5 - 4) * 0.08 - (5 - 4) * 0.02;
      if (newEF < 1.3) newEF = 1.3;
    } else {
      newReps = 0;
      newInterval = 1;
      newEF = Math.max(1.3, currentItem.ef - 0.2);
    }
  ...
  if (dueItems.length === 0) {
    return <div className="text-center text-gray-600 dark:text-gray-400">오늘 복습할 항목이 없습니다!</div>;
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300">오늘의 복습 항목 ({dueItems.length}개)</h2>
        <button
      ...
      {dueItems.map(item => (
        <div key={item.id} className="p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-700">
