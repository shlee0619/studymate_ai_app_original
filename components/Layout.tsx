    ...
    <div className="flex flex-col h-screen max-w-2xl mx-auto bg-white dark:bg-gray-900 shadow-2xl shadow-indigo-900/20 dark:shadow-indigo-900/50">
      ...
      <footer className="sticky bottom-0 left-0 right-0 bg-gray-50/80 dark:bg-gray-900/80 backdrop-blur-sm border-t border-gray-200 dark:border-gray-700">
        <nav className="flex justify-around max-w-2xl mx-auto">
          <NavItem label="대시보드" icon={navIcons.dashboard} isActive={activeScreen === 'dashboard'} onClick={() => onScreenChange('dashboard')} />
          <NavItem label="노트" icon={navIcons.library} isActive={activeScreen === 'library'} onClick={() => onScreenChange('library')} />
          <NavItem label="퀴즈" icon={navIcons.quiz} isActive={activeScreen === 'quiz'} onClick={() => onScreenChange('quiz')} />
          <NavItem label="복습" icon={navIcons.review} isActive={activeScreen === 'review'} onClick={() => onScreenChange('review')} />
          <NavItem label="오답" icon={navIcons.errors} isActive={activeScreen === 'errors'} onClick={() => onScreenChange('errors')} />
          <NavItem label="개념" icon={navIcons.concepts} isActive={activeScreen === 'concepts'} onClick={() => onScreenChange('concepts')} />
          <NavItem label="설정" icon={navIcons.settings} isActive={activeScreen === 'settings'} onClick={() => onScreenChange('settings')} />
        </nav>
      </footer>
    </div>
  );
