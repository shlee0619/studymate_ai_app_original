...
const calculateSm2 = (item: Item, correct: boolean, confidence: number, latencyMs: number) => {
    const q = (correct ? 1 : 0) * (0.6 + 0.4 * confidence) * (1 - 0.2 * Math.min(1, latencyMs / 3000));
...
export const useQuiz = ({ apiBaseUrl }: UseQuizParams) => {
    ...
    const getNextItem = useCallback(async () => {
        ...
        const difficultyRanges = [[0, 0.33], [0.34, 0.66], [0.67, 1.0]];
        const [min, max] = difficultyRanges[arm];

        let candidates = quizItems.current.filter(item => item.difficulty >= min && item.difficulty <= max);
        if(candidates.length === 0){ // fallback to any item if bucket is empty
    ...
    const submitAnswer = useCallback(async (item: Item, answerIndex: number, confidence: number, latencyMs: number, errorTagIds: string[]) => {
        ...
        await db.updateItemScheduling(item.id, scheduling.ef, scheduling.intervalDays, scheduling.reps, scheduling.nextReview);
